# Rodrigo Garza García

International affairs, public administration and data analytics professional with more than 10 years of experience in strategic planning, public policy, institutional coordination, KPI development, performance evaluation and executive reporting.

I combine an international and public-sector perspective with business analysis and analytical tools—including Excel, Google Sheets, SQL, Python, Pandas and data visualization—to transform complex information into actionable recommendations for public and private decision-makers.

## Core strengths

- Strategic planning and public policy
- Institutional coordination and stakeholder management
- KPI development and performance evaluation
- Executive reporting and communication
- Business analysis and data visualization

## Featured projects

### ConnectaTel Customer Usage & Segmentation Analysis

Analyzed telecom customer behavior using Python and Pandas. Most customers were in the Medium Usage segment, highlighting opportunities to improve retention, Premium adoption, and loyalty among high-usage customers.

**Skills:** Python, Pandas, EDA, Customer Segmentation, Descriptive Statistics, Matplotlib and Seaborn.

[View project](https://github.com/rogarzag/connectatel-customer-usage-analysis)

### Urban Mobility and Economic Analysis in Latin America

Analyzed 2024 traffic and economic data from 15 Latin American cities. The project found no clear relationship between GDP per capita and congestion and identified Bogotá as a priority for transportation investment.

**Skills:** Python, Pandas, data cleaning, aggregation, INNER JOIN, EDA, Matplotlib and Seaborn.

[View project](https://github.com/rogarzag/latam-traffic-economic-analysis)

---

### MercadoLibre Funnel and Retention Analysis

Analyzed the conversion funnel and D7, D14, D21 and D28 retention across ten Latin American countries. The main drop occurred between product selection and cart addition, while retention declined sharply after the first week.

**Skills:** SQL, CTEs, funnel analysis, cohort analysis, retention analysis and data validation.

[[View project](https://docs.google.com/spreadsheets/d/1j0Ln3y3g58qErGPMvOTCVIhawGD_T5hNhAE8IP0xCd0/edit?usp=drive_link)](https://github.com/rogarzag/mercadolibre-funnel-retention-analysis)

---

### Financial Performance Analysis with SQL

Compared revenue, costs, gross profit, margins and marketing ROI across six countries. The United States showed the strongest performance, while Canada and the United Kingdom presented lower marketing efficiency.

**Skills:** SQL, aggregation, KPI calculation, financial analysis, ROI analysis and executive reporting.

[View project](https://docs.google.com/spreadsheets/d/1zbW1QQRTN6DHKjlaQLtbhdRJkuep0zOcQLt1rfbU6tQ/edit?usp=drive_link)

---

### Walmart Sales Executive Summary

Analyzed Walmart’s 2012 weekly sales using sales per square meter and department share. The project identified the most efficient departments and supported recommendations for inventory, space and budget allocation.

**Skills:** Google Sheets, data integration, pivot tables, KPI development, dashboards and business analysis.

[View project](https://docs.google.com/spreadsheets/d/15P8TlKwUUgREb7f8LxmbPWThjBbRs80QfFafzBWDNqs/edit?usp=drive_link)

---

### Sales Data Cleaning and Analysis in Google Sheets

Cleaned and standardized a quarterly sales dataset and created an executive summary with key commercial metrics. The analysis identified Tablet as the best-selling category, Monterrey as the leading city and November as the strongest month.

**Skills:** Google Sheets, data cleaning, text standardization, KPI calculation, visualization and executive reporting.

[View project](https://docs.google.com/spreadsheets/d/1M9tRx3VppFVSAu5MsX2jNK6GeHLqd8m2/edit?usp=drive_link&ouid=109549579990095951417&rtpof=true&sd=true)
